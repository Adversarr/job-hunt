...
- 基于概率模型初始化词表，迭代删除低概率 token
- 适合：需要精细控制词表大小的场景
- 代表模型：T5、ALBERT

**WordPiece**
- 类似 BPE，但合并依据是语言模型概率而非频次：`##ing` 前缀标记
- 适合：英文单词拆分、BERT 系列

**关键公式：**
```
# BPE 合并评分
score(pair) = freq(pair)  # 频次驱动

# Unigram 评分
score(token) = P(token | corpus)  # 概率驱动

# Embedding 查表
embedding = EmbeddingTable[token_id]  # [vocab_size, hidden_dim]
```
...
print(f"Tokens: {output.tokens}")  # ['深度', '学习', '模型', '训练']
print(f"IDs: {output.ids}")  # [1234, 5678, 9012, 3456]
```
...
| 方案 | 收益 | 代价 | 适用边界 |
|------|------|------|----------|
| **BPE** | 实现简单、训练快、对多语言鲁棒 | 词表需人工设定大小、低频词易 OOV | 通用场景、多语言模型（GPT/LLaMA/Qwen） |
| **Unigram** | 概率建模、可控制词表质量、删除低频 token | 训练慢、需要大规模语料初始化 | 需要精细词表控制的场景（T5） |
| **WordPiece** | 子词粒度合理、适合英文单词拆分 | 对中文支持差、需要 ## 前缀标记 | 英文为主的理解任务（BERT） |
| **字符级** | 无 OOV、词表极小（~100） | 序列极长、计算开销大、语义稀疏 | 极小词表、特殊领域符号 |
| **Tie Embedding** | 节省参数（~300MB@7B）、减少过拟合 | 输入输出共享限制表达能力 | 参数受限场景、小模型 |
| **独立 Embedding** | 表达能力强、可独立初始化 | 参数量翻倍、显存占用高 | 大模型（70B+）、领域适配 |

## 高频追问（至少 5 个）

### 1. Q: BPE 和 SentencePiece 的核心差异是什么？
**A:** BPE 是算法（按频次合并字符对），SentencePiece 是工具库（实现 BPE/Unigram，**直接在原始字节流上操作**，无需预分词）。SentencePiece 支持 end-to-end 训练，对多语言/空格敏感语言（中文/日文）更友好。DeepSeek/Qwen 都用 SentencePiece+BPE。

### 2. Q: Embedding 层是否会参与微调？为什么？
**A:** 
...
### 4. Q: 为什么有的模型用 150k 词表（Qwen），有的只用 32k（LLaMA）？
**A:**
...
### 5. Q: Tokenizer 训练时 min_frequency 设多少合适？
**A:**
- **推荐值**：2-5（取决于语料规模）
...
### 6. Q: Tie embedding 会影响模型效果吗？什么场景不建议 tie？
**A:**
...
### 7. Q: 如何判断 tokenizer 对特定领域的支持是否足够？
**A:**
```python
# 检查领域文本的 tokenization 效率
text = "心肌梗死患者的治疗方案包括..."
tokens = tokenizer.tokenize(text)
efficiency = len(text) / len(tokens)  # 字符/token 比率

# 理想值：中文 > 1.5, 英文 > 3.0
# 过低说明 tokenizer 不适配，需定制训练
```
- **指标**：字符/token 比率、OOV 率、特殊符号覆盖率
- **解决**：领域语料增量训练 tokenizer、添加领域特殊 token
...
### 口述
- **能流畅讲清楚**：
  1. BPE、Unigram、WordPiece 三者的算法原理和差异
  2. Embedding 层在微调时是否更新、为什么、什么场景需要更新
  3. 中文 tokenizer 相比英文 tokenizer 的特殊挑战和解决方案
  4. Tie embedding 的利弊和适用场景
  5. 如何评估 tokenizer 对特定领域的支持是否足够
